    ...
    public String validCanCheckin(int userId) {
        ...
        if (type.equals("节假日")) {
            ...
            if (now.isBefore(attendanceStart)) {
                ...
                return result ? "今日已完成上班考勤，无需重复考勤" : "允许上班考勤";
            }
        }
    }
...
//                            result = "高风险";
                            if ("高风险".equals(result)) {
                                ...
                                message.setSubject("员工 " + name + " 身处疫情高风险地区警告");
                        ...
                        log.error("执行异常", e);
                        throw new EmosException("获取当前所在签到地点疫情风险等级失败");
                    }
                }
                // 保存签到记录
    ...
    public ArrayList<HashMap> searchWeekCheckin(HashMap param) {
        ...
        range.forEach(value -> {
            ...
            String status = "";
            if (type.equals("工作日") && DateUtil.compare(value, DateUtil.date()) <= 0) {
                status = "缺勤";
                boolean flag = false;
            ...
            map.put("type", type);
            map.put("day", value.dayOfWeekEnum().toChinese("周"));
            list.add(map);
        });
        return list;
    }
